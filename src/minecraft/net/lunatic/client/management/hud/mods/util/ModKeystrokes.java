...
simport net.lunatic.client.management.hud.ScreenPosition;
		...
		if(ModToggle.loadEnabledFromFile("ModKeystrokes") == true) {
			...
			for(Key key : mode.getKeys()) {
				...
				fr.drawString(
						...
						key.isDown() ? Color.BLACK.getRGB() : 0xffffff
						);	
				...
				if(key.cps) {
					...
					GlStateManager.translate(pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2F , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4F, 1F);
						...
						if(key.getName().matches(key.LMB.getName())) {
							fr.drawString("CPS: " + getCPS(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2 , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4, 0xffffff);
						}
						
						if(key.getName().matches(key.RMB.getName())) {
							fr.drawString("CPS: " + getCPS2(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2 , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4, 0xffffff);
